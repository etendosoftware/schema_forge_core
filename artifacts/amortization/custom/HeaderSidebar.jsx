...
function MetricCard({ label, value, subtitle, tint = null }) {
  const tints = {
    green: { bg: 'bg-emerald-50', text: 'text-emerald-800', label: 'text-emerald-600', sub: 'text-emerald-600' },
    amber: { bg: 'bg-amber-50', text: 'text-amber-800', label: 'text-amber-600', sub: 'text-amber-600' },
    blue: { bg: 'bg-blue-50', text: 'text-blue-800', label: 'text-blue-600', sub: 'text-blue-500' },
    gray: { bg: 'bg-gray-50', text: 'text-gray-900', label: 'text-gray-500', sub: 'text-gray-400' },
  };
  const c = tints[tint] || tints.gray;
  return (
    <div className={`rounded-xl ${c.bg} p-3`}>
      <div className={`text-xs ${c.label} mb-0.5`}>{label}</div>
