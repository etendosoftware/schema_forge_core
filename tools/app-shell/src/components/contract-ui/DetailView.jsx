      ...
      <div className="flex-1 flex flex-col bg-white rounded-tl-2xl overflow-hidden min-h-0">
        ...
        <div className="flex-1 overflow-auto px-6 pb-6">
          ...
          <div className="max-w-full space-y-6">
            ...
            {/* Related Documents (below lines, before totals) */}
            {customTabs.length > 0 && customTabs.map(ct => {
              const TabComponent = ct.Component;
              return (
                <details key={ct.key} className="group mt-4 rounded-lg border border-border bg-white shadow-sm overflow-hidden">
                  <summary className="cursor-pointer select-none list-none px-3 py-1.5 bg-muted/30 border-b border-border/50 flex items-center gap-2 hover:bg-muted/50 transition-colors">
                    <svg className="h-3.5 w-3.5 text-muted-foreground transition-transform group-open:rotate-90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{ct.label}</span>
                  </summary>
                      ...
                      {!hasNotesValue && !showNotes ? (
                        ...
                        <div className="rounded-lg border border-border bg-white shadow-sm overflow-hidden">
                          ...
                          <textarea
                            ...
                            className="w-full text-sm px-3 py-2 resize-y focus:outline-none border-0 bg-transparent placeholder:text-muted-foreground/40"
                          />
                        </div>
                      )}
                    </div>
