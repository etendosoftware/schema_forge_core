...
export default function DocumentTotalsPanel({
  ...
  useEffect(() => {
    if (lines.length === 0 && pendingLine == null) setTotalDiscountOpen(false);
  }, [lines.length, pendingLine]);
      ...
      <div className="w-full max-w-xs text-sm">
        ...
        */}
        <div style={!(hasPerProductDiscount || totalDiscountOpen) ? { borderTopWidth: '0.5px', borderTopStyle: 'solid', borderTopColor: 'var(--border)' } : {}}>
          ...
          {/* Total discount — interactive (calculation wired up later) */}
          {totalDiscountOpen && (
            <>
              <div className="flex justify-between items-center py-2 px-2">
                <label className="flex items-center gap-1.5 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked
                    onChange={() => setTotalDiscountOpen(false)}
                    className="h-3.5 w-3.5 rounded border-gray-300 text-primary focus:ring-primary cursor-pointer"
                  />
                  <span className="whitespace-nowrap">{ui('totalDiscount')}</span>
                </label>
                <span className="tabular-nums text-muted-foreground">{fmt(0)}</span>
              </div>
