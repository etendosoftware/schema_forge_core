...
describe('usePurchaseOrderPdf', () => {
  ...
  it('accepts orderId, apiBaseUrl and token parameters', () => {
    assert.match(src, /usePurchaseOrderPdf\(orderId,\s*apiBaseUrl,\s*token\)/);
  });
  ...
  it('fetches header from the purchase-order header endpoint', () => {
    assert.match(src, /purchase-order\/header\/\$\{orderId\}/);
  });

  it('fetches lines from the purchase-order lines endpoint with parentId', () => {
    assert.match(src, /purchase-order\/lines\?parentId=\$\{orderId\}/);
  });
