...
// ---------------------------------------------------------------------------
// Build purchase order data for the template
// ---------------------------------------------------------------------------
async function buildPurchaseOrderData(orderId, base, token) {
  const [header, linesRaw, session] = await Promise.all([
    fetchJson(`${base}/purchase-order/header/${orderId}`, token),
    fetchAll(`${base}/purchase-order/lines?parentId=${orderId}`, token),
