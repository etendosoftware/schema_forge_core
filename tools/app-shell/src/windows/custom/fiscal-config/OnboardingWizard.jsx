...
export default function OnboardingWizard({ orgId, orgName, apiBaseUrl, onComplete, onGoHome }) {
  ...
  const [cert, setCert]                   = useState(null); // {name, validTo} once uploaded
  ...
  if (step === 'detail') {
    ...
    async function handleSaveDetail() {
      if (system === 'SII+TBAI') {
        ...
        if (results.every(r => r.status === 'fulfilled')) goTo('applied');
        return;
      }
      if (system === 'SII') {
    ...
    return (
      <ScreenLayout actions={
        <>
          <Button variant="outline" onClick={() => goTo('confirm')}>{ui('fiscal.onboarding.back')}</Button>
          <span className="flex-1" />
    ...
    return (
      <ScreenLayout actions={
        <>
          ...
          <span className="flex-1" />
          <Button onClick={createRecords} disabled={saving}>
            ...
            <div className="grid gap-2.5 grid-cols-3">
              {Object.values(TERRITORIES).map(territory => (
                <TerrCard
                  ...
                  onPick={(territoryId) => handleTerritorySelection(territoryId, { clearManualSystem: true })}
                />
              ))}
            </div>
          </div>
                ...
                {manualAllowedSystems.map(systemId => {
                  ...
                  return (
                    <button
                      key={sys.id}
                      type="button"
                      onClick={() => setManualSystem(sys.id)}
      ...
      <ScreenLayout actions={
        ...
        {t.askNational && (
          <>
            ...
            <div className="flex flex-col gap-2">
              <RadioRow checked={alsoNational === false} onClick={() => setAlsoNational(false)}
                label={ui('fiscal.onboarding.subq.tbai.label')}
                description={ui('fiscal.onboarding.subq.tbai.desc', { territory: t.name })} />
              <RadioRow checked={alsoNational === true} onClick={() => setAlsoNational(true)}
        ...
        {t.askVolume && (
          <>
            ...
            <div className="flex flex-col gap-2">
              <RadioRow checked={volume === 'low'} onClick={() => setVolume('low')}
                label={ui('fiscal.onboarding.subq.volume.low.label')}
                description={ui('fiscal.onboarding.subq.volume.low.desc')} />
              <RadioRow checked={volume === 'high'} onClick={() => { setVolume('high'); setLowChoice(null); }}
            ...
            {volume === 'low' && (
              <>
                ...
                <div className="flex flex-col gap-2">
                  <RadioRow checked={lowChoice === 'verifactu'} onClick={() => setLowChoice('verifactu')}
                    label={ui('fiscal.onboarding.subq.verifactu.label')}
                    description={ui('fiscal.onboarding.subq.verifactu.desc')} />
                  <RadioRow checked={lowChoice === 'sii'} onClick={() => setLowChoice('sii')}
    ...
    >
      ...
      <div className="space-y-6">
            ...
            <div className={`grid gap-2.5 ${items.length === 1 ? 'grid-cols-1' : items.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>
              {items.map(id => (
                <TerrCard
                  ...
                  onPick={(territoryId) => handleTerritorySelection(territoryId)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="border-t pt-5">
          ...
          <button
            type="button"
            onClick={() => { setManualSystem(null); goTo('manual'); }}
