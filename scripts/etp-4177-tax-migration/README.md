...
| `00b-insert-obtl.sql` | **Destructive (insert).** Inserts the OBTL 303/349 dataset (`obtl_tributarykey`, `obtl_tax_report`, `obtl_tax_report_group`, `obtl_tax_report_parameter`, `obtl_tax_parameter` — 17,335 rows) at `client='0'`. GOClient has **no** OBTL rows, so promotion can't cover it; this data comes from the dataset XML. Generated by `gen-obtl-inserts.cjs`. Run after `00`. Dry-run by default; `-v do_commit=1` to apply. |
