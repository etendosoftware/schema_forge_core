...
describe('AdvancedFilterBuilder', () => {
  ...
  describe('operator switch clears an incompatible value (ETP-5008)', () => {
    ...
    // ETP-4956 retired `inSet` from the operator list offered for fresh
    // enumLabel selections (enum values are now picked through the
    // DistinctEnumPicker multi-select instead — "Es" with several codes
    // ticked already covers "is any of"). That makes the inverse of the test
    // above — switching an existing row's operator TO inSet via the picker —
    // impossible to drive through the UI: there is no `opInSet` option to
    // click once the row already holds a `equals`/`notEqual` operator. This
    // guards that retirement instead.
    it('does not offer "Es cualquiera de" (inSet) as a fresh choice for enum columns', async () => {
      const user = userEvent.setup();
      const value = {
        rowOperator: 'and',
        conditions: [{ field: 'status', operator: 'equals', value: '' }],
    ...
    it('still clears the pair when switching away from "between" (regression guard)', async () => {
      ...
      // Numeric editors are `type="text"` + `inputMode="decimal"` (ETP-4956),
      // not `type="number"`, so this is a textbox, not a spinbutton.
      const input = screen.getByRole('textbox');
