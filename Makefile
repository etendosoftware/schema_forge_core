...
test-all-coverage: ## Run ALL unit tests (Node + Vitest) with coverage reports
	...
	node --test --experimental-test-coverage --test-reporter=lcov --test-reporter-destination=coverage/appshell-core-lcov.info $(shell find packages/app-shell-core/test packages/app-shell-core/src/auth/__tests__ packages/app-shell-core/src/i18n/__tests__ packages/app-shell-core/src/components/ui/__tests__ packages/app-shell-core/src/runtime/__tests__ packages/app-shell-core/src/lib/__tests__ -name '*.test.js' 2>/dev/null)
