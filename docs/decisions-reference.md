...
- `useWindowAccess(windowId)` (from `@etendosoftware/app-shell-core/auth`, re-exported in the generated app via `@/auth/AuthContext.jsx`'s barrel — `etendo_schema_forge`'s local file that just does `export * from '@etendosoftware/app-shell-core/auth';` — so the generated import path you'll actually see is `@/auth/AuthContext.jsx`, not the package specifier directly), resolving `"none" | "read-only" | "full"` from the `windowAccess` map fetched once, at role-selection time, from `GET /webhooks/SFWindowAccessMap`.
...
| `hideRecordCount` | boolean | `false` | — | Hides the small record-count badge next to the window title. Use this when the list's own `hook.items.length` doesn't represent a meaningful count for the window — e.g. a custom tree/hierarchy view where the generic list hook only ever sees one paginated batch of leaves, not the full materialized structure. |
...
**`dimensionsPanelFieldKeys` prop (ETP-4610):** `generatePageComponent` separately collects the same lines-entity `dimensionsPanel: true` field keys (`detailFields.filter(f => f.dimensionsPanel && f.visibility !== 'discarded').map(f => f.name)`) and forwards them to the generated `<DetailView dimensionsPanelFieldKeys={[...]} />` prop, via the `dimensionsPanelFieldKeysProp` builder (`jsonWrapIf`, only emitted when the array is non-empty — purely additive, no output change for any window with zero dimensionsPanel fields). This is a **separate** consumer from `buildDimensionsPanelColumn`/`generateTableComponent` above (which builds the synthetic grid column) — `dimensionsPanelFieldKeys` instead lets the functional repo's `DetailView.jsx` widen, **scoped to that one window instance**, which visibility keys its dimension-macro filtering (`lineHiddenColumns`) is willing to trust, without touching the small global allowlist baked into that component (`DIMENSION_MACRO_KEYS`). See the functional repo's `docs/decisions-reference.md` (same `dimensionsPanel` section) for the full rationale — `product` is a real per-line AD field with its own record-dependent `displayLogic` on `sales-invoice`/`purchase-invoice` (ETP-4530), so it can never join the global allowlist, but `simple-g-l-journal` genuinely needs it trusted as a dimension macro, hence the per-window prop instead of a global fix.
