...
1. Open `/sales-invoice` and confirm the list shows exactly Invoice Date (no dot), Document No., Due Date (with red/amber/green dot), Business Partner, Document Status (AD status badge only — no payment-derived pills), Total Gross Amount, and Total Outstanding in that order. Confirm the Due Date shows "—" for invoices without a payment plan, a red dot for past due dates, an amber dot for due-today dates, and a green dot for future due dates. Also confirm date-only values such as an immediate-payment invoice created today render on the same calendar day instead of shifting backward by one day.
