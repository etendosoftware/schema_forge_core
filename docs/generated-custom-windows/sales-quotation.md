...
- The kebab (⋮) menu now exposes only the destructive `Reject` action (visible when `documentStatus === 'UE'`). Both the previous `Duplicate` entry (redundant with the topbar `Clonar` button) and the previous `Cancel` placeholder (wired to an empty `onClick` and never functional) were removed.
