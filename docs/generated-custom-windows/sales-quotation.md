...
- Discount panel: the `DocumentTotalsPanel` in the detail footer always keeps the `discount` column visible in the lines grid and add-row — there is no toggle. "Subtotal sin descuento" and "Descuento por producto" rows auto-appear when `discountAmt > 0` (at least one line has a non-zero discount); both are read-only computed rows. A `+ Añadir descuento total` button appears below the totals when no total discount is active and at least one line exists; clicking it opens an interactive "Descuento total" section with a checkbox, computed amount, and a percentage input. Unchecking the checkbox collapses the section and restores the button. The total-discount calculation is a UI placeholder only — no backend persistence yet.
