...
- Discount panel: clicking `+ Añadir descuento` in the detail footer expands a breakdown with a "Descuento por producto" checkbox that shows or hides the `discount` column in the lines grid and add-row. The panel auto-expands on load when any existing line has a non-zero discount, and collapses back to simple mode when all lines are deleted.
