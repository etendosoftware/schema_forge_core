...
- Returned quantity, unit price, line net amount, and tax are wired to amount-related callouts in the contract, and the header exposes read-only net and gross totals. This suggests totals should react to line changes, but the exact recalculation timing and discount behavior are not directly shown in the current evidence.
