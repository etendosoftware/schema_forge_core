...
- **Left panel — pending statement lines** (`usePendingStatementLines(accountId, filters)`): a toolbar (`Pendiente (N)` chip + `Últimos 12 meses` chip + search), a table with **radio single-select** rows (Fecha · Descripción + status badge · Importe with sign tone), and a `Total: X,XX €` footer.
- **Right panel — candidate operations** (`useCandidateOperations(accountId, lineId, docType)` — does NOT fetch while no line is selected): an empty state (`Selecciona un movimiento` / hint) until a line is picked, then a `SelectedLineHeader` (line metadata + amount in red/green), a docType/date/search toolbar, and a table with **checkbox multi-select** rows (Fecha · Información = documentNo + partnerName + badge · Saldo pendiente · Importe). Backend-suggested candidates carry a blue **"Sugerida"** badge; the rest "Pendiente".
...
- `Automatch` (top-right) is rendered **disabled** (enabled in T7). `Transferir` / `Nuevo documento` render but fire a "próximamente" toast (follow-up).
