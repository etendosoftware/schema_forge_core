...
- The one-price-list-one-version expectation in Etendo Go is sustained by `PriceListEventHandler` (auto-creates the first version) and the GO UI not exposing a version selector. It is intentionally not hard-enforced at the persistence layer, so Etendo Classic / Enterprise users keep their ability to manage multiple versions. `PriceListVersionResolver` returns the first version and logs a warning if more than one is found.
...
- `modules/com.etendoerp.go/src/.../PriceListVersionResolver.java` centralizes the single-version lookup used by the handler and warns when more than one version is found, without blocking Classic functionality.
