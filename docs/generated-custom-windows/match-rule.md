...
- The window is not yet wired into `menu.json`/the registry, and the spec/entity is created by `push-to-neo` (run by the user). Until those steps run, the route is not reachable in the SPA.
...
3. Create a Regex-condition rule with a deliberately catastrophic pattern (e.g. `(a+)+$`) and confirm the save is rejected with a 400 error message.
