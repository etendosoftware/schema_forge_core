...
- Line pricing follows `INVOICE_LINE_CONFIG` (see `docs/line-pricing-model.md`). The editable price field is `listPrice` (PriceList column). `unitPrice` (PriceActual) is hidden and derived server-side. There is no discount field. The product selector fetches the correct price-list price directly; the `SL_Invoice_Product` callout's `listPrice` response value is discarded by `DetailView.jsx` because it comes from product defaults and does not reflect the invoice's price list context. `lineGrossAmount` (exposed here as `grossAmount`) is computed client-side as `invoicedQuantity × listPrice × taxFactor`. The `decisions.json` declares `lineEntityConfig: "invoice"`, which drives all of these behaviors via the generator and `DetailView.jsx`.
