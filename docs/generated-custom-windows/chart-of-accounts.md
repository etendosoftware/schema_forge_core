...
- Create a new account with the editable setup fields visible in the generated form: Code, Name, Account Type, and optional Parent Account.
- Open an existing account and update those same setup fields.
...
- Window shape: single-entity window for `account`; no master-child layout or child tabs are present in the current contract or generated UI.
...
- New-record defaulting is handled by the shared generated entity flow in the app shell when a backend defaults endpoint exists, but this window's contract does not document any account-specific defaults.

## Gap assessment
- A chart-of-accounts screen often carries stricter accounting semantics such as posting-vs-summary restrictions, protected parent/child hierarchy rules, account-type-specific behavior, or constraints on deleting accounts with activity. None of those rules are visible in the current contract or generated UI, so they remain gaps or open ambiguities.
