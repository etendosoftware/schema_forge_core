...
- Warehouse is still treated as required by backend and E2E evidence, but the generated header form inspected on `origin/develop` still does not expose it. That leaves an open question about how a user supplies or changes warehouse during order entry in the current generated header surface.
- `scheduledDeliveryDate`, `invoiceTerms`, and `invoiceAddress` still exist in backend/detail evidence but remain discarded or hidden from the generated header form. Payment terms is no longer part of that hidden set; the other order-entry semantics still are.
...
- The menu action shows `Cancel` only when status is `CO`, but the generated page still wires it with an empty `onClick`. The action is visible in configuration, yet its actual user-facing cancellation behavior is not evidenced here.
...
3. Start a new order and verify business partner is required, partner address stays disabled until a business partner is selected, the order-line tab is already visible, and `Save`, `Save draft`, and `Cancel` are present before the first save.
4. Verify the header now exposes `Payment Method` and `Payment Terms` as visible fields on the generated header form.
5. Add one or more lines and verify the line editor exposes product, ordered quantity, net unit price, discount, tax, and line gross amount, then verify the header totals refresh after saving line changes.
