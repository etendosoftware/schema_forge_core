...
- The inspected code does not derive the Company/Person toggle from saved business-partner data when the window opens. The local contacts context initializes to Company, so existing person-like records may reopen in Company mode until the user flips the toggle. This is observable in code, but the exact runtime UX still needs manual confirmation.
...
- The Company/Person toggle is a UI-only state in the inspected code. No saved `contactType`/persona field or record-load hydration was found, so the exact persistence model for person-vs-company presentation remains ambiguous.
