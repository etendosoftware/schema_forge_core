...
- Add one or more journal lines under a header, each with an account, an optional description, a debit amount, and a credit amount. The line `Description` carries the AD default `@DESCRIPTION1@` (the tab auxiliary input `DESCRIPTION1` = the parent journal's description). NEO Headless now resolves this auxiliary input in its defaults pipeline, so the line `/defaults` response carries the header description. **Note:** the inline add-row UI does not yet consume the line `/defaults` endpoint, so the field is not pre-filled in the form pending that frontend change — see Gap assessment.
...
- **Line description pre-fill (deferred).** The backend resolves `@DESCRIPTION1@` to the header description in the line `/defaults` response (via `NeoAuxiliaryInputResolver`), but the inline add-row UI does not yet fetch `/{detailEntity}/defaults?parentId=…`, so the field opens empty. Completing the pre-fill needs a generic frontend change (fetch line defaults on add-row open and seed editable fields) — tracked separately under ETP-4244.
