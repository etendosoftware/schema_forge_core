...
`decisions.json` files carry a `version` field (number) and a `$schema` tag (e.g., `decisions-v1`). When the decisions structure changes, migration scripts in `cli/src/migrations/` transform old formats automatically.
