...
${secondaryTabDefs.length > 0 ? `${secondaryTabsImports}\n` : ''}${relatedDocsImport}${customCompImportBlock}import catalogs from './mockCatalogs';
  ...
  if (recordId) {
    return (
      <DetailView
        ...
        recordId={recordId}
        breadcrumb={breadcrumb}${apiProp}${detailTabIndexProp}${secondaryTabsProp}${documentPreviewProp}${hideDeleteProp}${notesFieldProp}${customTabsProp}${customCompPropsBlock}${menuActionsProp}${draftModeProp}${headerContentProp}${detailSortByProp}
        {...props}
      />
    );
  }
