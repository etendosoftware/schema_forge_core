...
function resolveSecondaryTabDefs(secondaryTabsDecl, contract, headerEntity, detailEntity, headerColumnMap, headerBooleanFields) {
  ...
  return Object.entries(secondaryTabsDecl)
    ...
    .map(([key, cfg]) => {
      ...
      return { key, label: cfg.label ?? toLabel(key), isFormTab, isPanelTab, isCustomForm: !!cfg.customForm, isCustomTable: !!cfg.customTable, PanelName, FormName, TableName, addLineEntries, requireSavedRecord, maxDetailLines, isCustomAddModal: !!customAddModalName, CustomAddModalName: customAddModalName, readOnlyLogicJs, excludeFields };
    });
}
...
function buildSecondaryTabPropEntry(t) {
  ...
  return `          { key: '${t.key}', label: '${t.label}', Table: ${t.TableName}${formProp}${excludeFieldsPart}${addLinePart}${customAddModalPart}${requireSavedPart}${readOnlyLogicPart}${maxDetailLinesPart} },`;
}
