 ...
 * LEFT JOINs AD_Form to get the form classname when action='X'.
 */
...
export function resolveFromRow(row) {
  ...
  if (action === 'X') {
    const classnameHint = formClassname
      ? `src/org/openbravo/erpCommon/ad_forms/${formClassname}.java`
      : `(unknown — check AD_Form for this menu entry)`;

    throw new Error(
      `Form detected: '${menuName}' is an AD_Form (action='X').\n\n` +
      `Forms are custom implementations that cannot be auto-generated by the pipeline.\n` +
      `See docs/form-migration-guide.md for the step-by-step migration process.\n\n` +
      `Summary:\n` +
      `  1. Analyze legacy source: ${classnameHint}\n` +
      `  2. Create a NeoHandler (Java) with custom logic\n` +
      `  3. Register in NEO Headless (ETGO_SF_* tables)\n` +
      `  4. Create a custom React component\n` +
      `  5. Register in app-shell registry + menu\n\n` +
      `This is artisanal work — each form is unique.`
    );
  }
