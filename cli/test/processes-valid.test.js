...
describe('processes.json', () => {
  it('is valid against processes.schema.json', { skip: !hasProcesses && 'processes.json not found' }, async () => {
  ...
  it('defines completeOrder with >= 3 edge cases', { skip: !hasProcesses && 'processes.json not found' }, async () => {
    const data = JSON.parse(await readFile(PROCESSES_PATH, 'utf8'));
  ...
  it('defines voidOrder with >= 3 edge cases', { skip: !hasProcesses && 'processes.json not found' }, async () => {
    const data = JSON.parse(await readFile(PROCESSES_PATH, 'utf8'));
  ...
  it('all steps use only validate|mutation|forEach types', { skip: !hasProcesses && 'processes.json not found' }, async () => {
    const data = JSON.parse(await readFile(PROCESSES_PATH, 'utf8'));
