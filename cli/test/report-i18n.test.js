 ...
 */
const UI_KEYS = [
  'records', 'total', 'totals', 'filters', 'generatedBy', 'generatedOn',
  'printedOn', 'page', 'initialBalance', 'subtotal', 'yes', 'no',
];
...
describe('REPORT_UI_STRINGS', () => {
  ...
  it('carries the generatedOn "Generated on"/"Generado el" label (ETP-5013)', () => {
    assert.equal(REPORT_UI_STRINGS.en_US.generatedOn, 'Generated on');
    assert.equal(REPORT_UI_STRINGS.es_ES.generatedOn, 'Generado el');
  });

  it('keeps generatedOn distinct from generatedBy — different strings, different purpose', () => {
    // generatedBy ("Generated by Etendo Go" / "Generado por Etendo Go") is the
    // report's system-authorship footer; generatedOn is the render-date label
    // that now prefixes every report's timestamp. Easy to conflate by name —
    // pinned here so a future edit can't silently merge them.
    for (const locale of Object.keys(REPORT_UI_STRINGS)) {
      assert.notEqual(REPORT_UI_STRINGS[locale].generatedOn, REPORT_UI_STRINGS[locale].generatedBy);
    }
  });

  it('carries the printedOn "Printed on"/"Impreso el" label for the PDF page footer (ETP-5013)', () => {
    assert.equal(REPORT_UI_STRINGS.en_US.printedOn, 'Printed on');
    assert.equal(REPORT_UI_STRINGS.es_ES.printedOn, 'Impreso el');
  });

  it('carries the page "Page"/"Página" label for the PDF page footer (ETP-5013)', () => {
    assert.equal(REPORT_UI_STRINGS.en_US.page, 'Page');
    assert.equal(REPORT_UI_STRINGS.es_ES.page, 'Página');
  });

  it('keeps printedOn and page distinct from each other and from generatedBy/generatedOn', () => {
    for (const locale of Object.keys(REPORT_UI_STRINGS)) {
      const strings = REPORT_UI_STRINGS[locale];
      const footerKeys = ['printedOn', 'page', 'generatedBy', 'generatedOn'];
      const values = footerKeys.map((k) => strings[k]);
      assert.equal(new Set(values).size, values.length, `${locale} has duplicate footer/chrome label values`);
    }
  });
});
